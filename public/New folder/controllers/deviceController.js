...
exports.listDeviceInfo = async (req, res) => {
    try {
      ...
      const listedData = await deviceModel.find(whereCodn).populate({
        path: 'user_id',
        model: 'AppUsers',
        select: { 'f2A_creds': 0,'password':0,'emailVerificationCode':0,'mobileVerificationCode':0,"forgot_otp":0,"signup_type":0},
