...
function InputDetail({ settabel }) {
    ...
    const [coin0erro, setCoin0error] = useState("")
    ...
    const submit = async () => {
        try {
            if (coin0.current.value === "") {
                setCoin0error("Please Enter LP Token")
    ...
    return (
        <>
            ...
            <div style={{ display: "flex", flexDirection: "column", width: "40%", margin: "auto", textAlign: "center", gap: "1em", padding: "1em" }}>
                ...
                {coin0erro !== "" ?
                    <span style={{ color: "red", fontSize: "15px" }}>{coin0erro}</span>
                    :
                    <></>
                }
